...
class BudgetLine(Base):
    ...
    regel_type: Mapped[str] = mapped_column(String(40), default="regel", index=True)
    niveau: Mapped[int] = mapped_column(Integer, default=0)
    hoofdstuk_code: Mapped[str | None] = mapped_column(String(80), nullable=True, index=True)
    hoofdstuk_omschrijving: Mapped[str | None] = mapped_column(Text, nullable=True)
    post_code: Mapped[str | None] = mapped_column(String(80), nullable=True, index=True)
...
class ReferenceLine(Base):
    ...
    bdb_indexering: Mapped[Decimal | None] = mapped_column(Numeric(12, 4), nullable=True)
    project_sheet_name: Mapped[str | None] = mapped_column(String(180), nullable=True, index=True)
